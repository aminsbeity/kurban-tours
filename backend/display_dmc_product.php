...
if($keywords != ""){
	$condition.=" dmc_product_title like '%$keywords%' and ";
}
...
$offset = 0;
$page=$_REQUEST["page"];
$recordsCount = Dmc_product::count($condition);
...
$records = Dmc_product::select($condition);
...
<div style="border:0px; padding:3px; background-color: #f3f3f3; font-weight: bold;color:#0569c5;font-size:18px;" align="center">dmc product Management</div>

<div style="background-color: #f3f3f3; font-size: 11px;">
	<form name="myform223" action="<?php echo $_SERVER['PHP_SELF'] ?>" method="post">
	<div>Search by dmc product title: <input type="text" value="<?php echo $keywords ?>" name="keywords" id="keywords"><input type="submit" value=" Search " /></div>
	</form>
</div>

<div style="">
	<a href="new_dmc_product.php"><img src="images/new.png" border="0"/></a>
</div>
<form name="myform" action="delete_dmc_product.php" method="post">
<table width="100%" border="0" cellspacing="0" cellpadding="0">
  <tr>
    <td width="35" style="border-bottom:1px solid #e5eaef;">&nbsp;</td>
    <td width="100" class="TdHeaders TdHeaderBorder" >Dmc product id</td>
	<td class="TdHeaders TdHeaderBorder">Dmc product title<br/><a href="?orderBy=asc&fieldName=Dmc_product_title&page=<?php echo $page?>&keywords=<?php echo $keywords?>">Sort Asc</a>|<a href="?orderBy=desc&fieldName=Dmc_product_title&page=<?php echo $page?>&keywords=<?php echo $keywords?>">Sort Desc</a></td>
	
    <td colspan="2" class="TdHeaders TdHeaderBorder">Actions</td>
  </tr>
  <?
  for($rc=0;$rc<count($records);$rc++){
  $dmc_product = $records[$rc];?>
  <tr>
    <td width="35" class="TdBodyborder" ><input type="checkbox" value="<?=$dmc_product->dmc_product_id;?>" name="check_list<?=$rc;?>"/></td>
    <td width="100" class="TdBodyborder"><?=$dmc_product->dmc_product_id;?>&nbsp;</td>
    <td class="TdBodyborder"><?=$dmc_product->dmc_product_title;?>&nbsp;</td>

    <td width="50" align="center" style="border-bottom:1px solid #e5eaef;border-Left:1px solid #e5eaef;" ><a href="edit_dmc_product.php?id=<?=$dmc_product->dmc_product_id?>"><img src="images/edit.png"  height="25" border="0"></a></td>
