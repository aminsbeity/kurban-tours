...
if($keywords != ""){
	$condition.=" partner_name like '%$keywords%' and ";
}
...
$offset = 0;
$page=$_REQUEST["page"];
$recordsCount = Partner::count($condition);
...
$records = Partner::select($condition);
...
<div style="border:0px; padding:3px; background-color: #f3f3f3; font-weight: bold;color:#0569c5;font-size:18px;" align="center">partner Management</div>

<div style="background-color: #f3f3f3; font-size: 11px;">
	<form name="myform223" action="<?php echo $_SERVER['PHP_SELF'] ?>" method="post">
	<div>Search by partner name: <input type="text" value="<?php echo $keywords ?>" name="keywords" id="keywords"><input type="submit" value=" Search " /></div>
	</form>
</div>

<div style="">
	<a href="new_partner.php"><img src="images/new.png" border="0"/></a>
</div>
<form name="myform" action="delete_partner.php" method="post">
<table width="100%" border="0" cellspacing="0" cellpadding="0">
  <tr>
    <td width="35" style="border-bottom:1px solid #e5eaef;">&nbsp;</td>
    <td width="100" class="TdHeaders TdHeaderBorder" >Partner id</td>
	<td class="TdHeaders TdHeaderBorder">Partner name<br/><a href="?orderBy=asc&fieldName=Partner_name&page=<?php echo $page?>&keywords=<?php echo $keywords?>">Sort Asc</a>|<a href="?orderBy=desc&fieldName=Partner_name&page=<?php echo $page?>&keywords=<?php echo $keywords?>">Sort Desc</a></td>
  ...
  $partner = $records[$rc];?>
  <tr>
    <td width="35" class="TdBodyborder" ><input type="checkbox" value="<?=$partner->partner_id;?>" name="check_list<?=$rc;?>"/></td>
    <td width="100" class="TdBodyborder"><?=$partner->partner_id;?>&nbsp;</td>
    <td class="TdBodyborder"><?=$partner->partner_name;?>&nbsp;</td>

    <td width="50" align="center" style="border-bottom:1px solid #e5eaef;border-Left:1px solid #e5eaef;" ><a href="edit_partner.php?id=<?=$partner->partner_id?>"><img src="images/edit.png"  height="25" border="0"></a></td>
