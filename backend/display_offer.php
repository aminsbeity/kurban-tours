...
if (trim($_REQUEST["fromLink"]) != ""){
	$_SESSION['choosedDestination']="";
}
if ($destinationId != ""){
	$_SESSION['choosedDestination']=$destinationId;
}else{
	$destinationId=$_SESSION['choosedDestination'];
}

if($keywords != ""){
	$condition.=" offer_title like '%$keywords%' and ";
}

if ($destinationId != ""){
	$condition.=" destination_id = $destinationId and ";
...
$offset = 0;
$page=$_REQUEST["page"];
$recordsCount = Offer::count($condition);
...
$records = Offer::select($condition);
...
<div style="border:0px; padding:3px; background-color: #f3f3f3; font-weight: bold;color:#0569c5;font-size:18px;" align="center">offer Management</div>

<div style="background-color: #f3f3f3; font-size: 11px;">
	<form name="myform223" action="<?php echo $_SERVER['PHP_SELF'] ?>" method="post">
	<div>Search by offer title: <input type="text" value="<?php echo $keywords ?>" name="keywords" id="keywords"><input type="submit" value=" Search " /></div>
	</form>
</div>

<div style="">
	<a href="new_offer.php"><img src="images/new.png" border="0"/></a>
</div>
<form name="myform" action="delete_offer.php" method="post">
<table width="100%" border="0" cellspacing="0" cellpadding="0">
  <tr>
    <td width="35" style="border-bottom:1px solid #e5eaef;">&nbsp;</td>
    <td width="100" class="TdHeaders TdHeaderBorder" >Offer id</td>
	<td class="TdHeaders TdHeaderBorder">Offer date<br/><a href="?orderBy=asc&fieldName=Offer_date&page=<?php echo $page?>&keywords=<?php echo $keywords?>">Sort Asc</a>|<a href="?orderBy=desc&fieldName=Offer_date&page=<?php echo $page?>&keywords=<?php echo $keywords?>">Sort Desc</a></td><td class="TdHeaders TdHeaderBorder">Offer title<br/><a href="?orderBy=asc&fieldName=Offer_title&page=<?php echo $page?>&keywords=<?php echo $keywords?>">Sort Asc</a>|<a href="?orderBy=desc&fieldName=Offer_title&page=<?php echo $page?>&keywords=<?php echo $keywords?>">Sort Desc</a></td>
  ...
  $offer = $records[$rc];?>
  <tr>
    <td width="35" class="TdBodyborder" ><input type="checkbox" value="<?=$offer->offer_id;?>" name="check_list<?=$rc;?>"/></td>
    <td width="100" class="TdBodyborder"><?=$offer->offer_id;?>&nbsp;</td>
    <td class="TdBodyborder"><?=$offer->offer_date;?>&nbsp;</td>
<td class="TdBodyborder"><?=$offer->offer_title;?>&nbsp;</td>

    <td width="50" align="center" style="border-bottom:1px solid #e5eaef;border-Left:1px solid #e5eaef;" ><a href="edit_offer.php?id=<?=$offer->offer_id?>"><img src="images/edit.png"  height="25" border="0"></a></td>
