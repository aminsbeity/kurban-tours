...
if($keywords != ""){
	$condition.=" latest_news_title like '%$keywords%' and ";
}
...
$offset = 0;
$page=$_REQUEST["page"];
$recordsCount = Latest_news::count($condition);
...
$records = Latest_news::select($condition);
...
<div style="border:0px; padding:3px; background-color: #f3f3f3; font-weight: bold;color:#0569c5;font-size:18px;" align="center">latest news Management</div>

<div style="background-color: #f3f3f3; font-size: 11px;">
	<form name="myform223" action="<?php echo $_SERVER['PHP_SELF'] ?>" method="post">
	<div>Search by latest news title: <input type="text" value="<?php echo $keywords ?>" name="keywords" id="keywords"><input type="submit" value=" Search " /></div>
	</form>
</div>

<div style="">
	<a href="new_latest_news.php"><img src="images/new.png" border="0"/></a>
</div>
<form name="myform" action="delete_latest_news.php" method="post">
<table width="100%" border="0" cellspacing="0" cellpadding="0">
  <tr>
    <td width="35" style="border-bottom:1px solid #e5eaef;">&nbsp;</td>
    <td width="100" class="TdHeaders TdHeaderBorder" >Latest news id</td>
	<td class="TdHeaders TdHeaderBorder">Latest news title<br/><a href="?orderBy=asc&fieldName=Latest_news_title&page=<?php echo $page?>&keywords=<?php echo $keywords?>">Sort Asc</a>|<a href="?orderBy=desc&fieldName=Latest_news_title&page=<?php echo $page?>&keywords=<?php echo $keywords?>">Sort Desc</a></td>
	
    <td colspan="2" class="TdHeaders TdHeaderBorder">Actions</td>
  </tr>
  <?
  for($rc=0;$rc<count($records);$rc++){
  $latest_news = $records[$rc];?>
  <tr>
    <td width="35" class="TdBodyborder" ><input type="checkbox" value="<?=$latest_news->latest_news_id;?>" name="check_list<?=$rc;?>"/></td>
    <td width="100" class="TdBodyborder"><?=$latest_news->latest_news_id;?>&nbsp;</td>
    <td class="TdBodyborder"><?=$latest_news->latest_news_title;?>&nbsp;</td>

    <td width="50" align="center" style="border-bottom:1px solid #e5eaef;border-Left:1px solid #e5eaef;" ><a href="edit_latest_news.php?id=<?=$latest_news->latest_news_id?>"><img src="images/edit.png"  height="25" border="0"></a></td>
